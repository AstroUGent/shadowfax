...
/**
 * @file DelTessCheck.cpp
 *
 * @brief File containing a method to check the Delaunay tesselation algorithm
 *
 * This file only contains a testing routing for the insertion and flipping
 * algorithms. All tests are hardcoded. They depend upon the structure of the
 * DelTess, but the results should be independent of the implementation of the
 * ghost points etc.
 *
 * The tests do not fully cover all possible things that could go wrong, but
 * they are useful when adjusting things to the insertion algorithm, to make
 * sure that there's no big bug in the algorithm.
 *
