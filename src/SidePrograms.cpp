 ...
 */
void SidePrograms::load_ascii(ParticleVector& cells,
                              double* cube_boundaries, string name){
    ...
    for(unsigned int i = block.get_size(); i--;){
        ...
        for(unsigned int j = ndim_; j--;){
            ...
            cube_boundaries[2*j+1] = std::max(cube_boundaries[2*j+1], row[j]);
        }
...
AreaCalculator::AreaCalculator(int argc, char **argv) : SidePrograms(){
    ...
    side *= 1.02;
    Vec origin(cube_boundaries[0]+0.5*side[0], cube_boundaries[2]+0.5*side[1]);
    if(custom_side){
    ...
    for(unsigned int i = particle_list.gassize(); i--;){
        particle_list.gas(i)
                ->set_key(cube.get_key(particle_list.gas(i)->get_position()));
    }
    particle_list.sort();
...
DensityCalculator::DensityCalculator(int argc, char **argv) : SidePrograms(){
    ...
    optind = 1;
    opterr = 0;
    while((c = getopt_long(argc, argv, ":f:", long_options, NULL))
          != -1){
        switch(c){
        case 'f':
            filename = optarg;
    ...
    for(unsigned int i = particle_list.gassize(); i--;){
        ...
        for(unsigned int j = ndim_; j--;){
            bits[j] = ((particle_list.gas(i)->pos(j)-
                        cube_boundaries[2*j])/side[j])*width;
        }
...
HilbertSorter::HilbertSorter(int argc, char **argv){
    ...
    while((c = getopt_long(argc, argv, ":f:", long_options, NULL))
        ...
        case 'f':
            filename = optarg;
            // drop the last 5 characters from the name by setting the end of
            // string
...
MassCalculator::MassCalculator(int argc, char **argv){
    ...
    optind = 1;
    opterr = 0;
    while((c = getopt_long(argc, argv, ":f:t:", long_options, NULL))
          != -1){
        switch(c){
        case 'f':
            filename = optarg;
    ...
    SnapshotReader *reader = SnapshotReaderFactory::generate(type, filename,
                                                             simulation_units);
