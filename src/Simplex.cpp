...
VorGen* Simplex::get_special_point(vector<VorGen*>& points){
    ...
    if(_midpoint_circumsphere == NULL){
        ...
        if(V < 1.e-13){
            Vec position;
            position[0] = 0.25*(points0->x() + points1->x() +
                                points2->x() + points3->x());
            position[1] = 0.25*(points0->y() + points1->y() +
                                points2->y() + points3->y());
            position[2] = 0.25*(points0->z() + points1->z() +
                                points2->z() + points3->z());
            delete _midpoint_circumsphere;
            _midpoint_circumsphere = new VorGen(position[0], position[1],
                                                position[2]);
        } else {
            double Vinv = 1./V;
...
VorGen* Simplex::get_special_point(vector<VorGen*>& points){
    if(_midpoint_circumsphere == NULL){
        ...
        R[0] = (b[1]*a2 - a[1]*b2)/D;
        R[1] = (a[0]*b2 - b[0]*a2)/D;
        _midpoint_circumsphere = new VorGen(points0->x() + R[0],
                                            points0->y() + R[1]);
    }
    return _midpoint_circumsphere;
}
