...
double Simulation::load(ParticleVector& cells, string ictype, string filename,
    ...
    SnapshotReader *reader = SnapshotReaderFactory::generate(ictype, filename,
                                                             simulation_units);
    Header header = reader->read_snapshot(cells);
    cout << "Found " << header.npart() << " particles (gas: "
         << header.ngaspart() << ", DM: " << header.ndmpart() << ")" << endl;
...
void Simulation::main_loop(){
    ...
    while(_timeline->step_forward()){
        cout << "t = " << _timeline->get_time() << ", dt = "
             << _timeline->get_realtime(dt) << endl;
        ...
        if(_particles->gassize()){
            for(unsigned int i = 0; i < _particles->gassize(); i++){
                if(_particles->gas(i)->get_starttime() == currentTime){
                    ...
                    Wgas += 0.5*_timeline->get_realtime(
            ...
            for(unsigned int i = _particles->dmsize(); i--;){
                if(_particles->dm(i)->get_starttime() == currentTime){
                    _particles->dm(i)
                            ->accelerate(
                                _particles->dm(i)
                                ->get_gravitational_acceleration()*0.5*
                ...
                if(_particles->gas(i)->get_endtime() == currentTime){
                    ...
                    _particles->gas(i)
                            ->set_gravitational_acceleration(
                                _particles->gas(i)
                                ->get_gravitational_acceleration()+acorr
                                );
#endif
                }
            }
...
#else
                StateVector W = _solver->get_W(V,
                                               _particles->gas(i)->get_Qvec());
#endif
...
void Simulation::initialize_MPI_buffer(unsigned int partsize,
    ...
    if(maxsize < (sizeof(GasParticle) + sizeof(int))*(MPIGlobal::size-1)){
        cerr << "Communication buffer too small for a single GasParticle!\n"
                "Try increasing the maximum size of the buffer!" << endl;
        my_exit();
    }
...
void Simulation::restart(string filename){
    ...
    _physics = new Physics(*_simulation_units,
                           _parameterfile->has_real_physics());
...
void Simulation::check_for_restart(bool force_restart){
    ...
    if(check || force_restart){
        ...
        RestartFile rfile(_parameterfile->get_outputdir(),
                          _timeline->get_realtime(
...
void Simulation::initialize(string filename){
    ...
    _solver = RiemannSolverFactory::generate(_parameterfile->get_solver_type(),
                                             _parameterfile->get_gamma(),
                                             _parameterfile->get_tolerance(),
                                             _parameterfile->get_cutoff());
    _simulation_units = UnitSetGenerator::generate(
                _parameterfile->get_units_internal()
                );
    _physics = new Physics(*_simulation_units,
                           _parameterfile->has_real_physics());
    LOGS("Simulation UnitSet created");
    _output_units = UnitSetGenerator::generate(
                _parameterfile->get_units_output()
                );
    LOGS("Output UnitSet created");
    _box = new RectangularBox();
    _particles = new ParticleVector(
                _parameterfile->get_global_timestep(), *_box, _periodic,
                _periodic && _parameterfile->has_gravity(),
                _parameterfile->get_ewald_alpha(),
                _parameterfile->get_ewald_size()
                );
    LOGS("ParticleVector created");
    _timeline = new TimeLine(_parameterfile->get_maxtime(),
                             _parameterfile->get_snaptime(),
                             _parameterfile->get_CFL(),
                             _parameterfile->get_grav_eta(),
                             *_particles, _parameterfile->get_snapshot_type(),
                             _parameterfile->get_outputdir() + string("/") +
                             _parameterfile->get_basename(),
                             *_simulation_units, *_output_units,
                             _parameterfile->has_gravity(), _periodic,
                             _parameterfile->has_treetime(),
                             _parameterfile->get_max_timestep(),
                             _parameterfile->get_min_timestep(),
                             _parameterfile->get_firstsnap(),
