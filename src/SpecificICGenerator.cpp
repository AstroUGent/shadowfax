...
void SpecificICGenerator::make_cartesian_grid(ParticleVector& plist){
    ...
    unsigned int numpart = _ngaspart/MPIGlobal::size +
            ((_ngaspart%MPIGlobal::size) > 0);
...
//    ofstream ofile("particles.dat");
    ...
    for(unsigned int i = 0; i < numpart; i++){
        ...
        // afterwards
        if(_type == IC_SPEC_EVRARD && numinside < (1.-2.*_evrardfrac/3.)/
                (32.*_evrardfrac/M_PI + 1. - 2.*_evrardfrac/3.)*_ngaspart){
...
void SpecificICGenerator::apply_profile_kh(ParticleVector &grid){
    ...
    for(unsigned int i = grid.gassize(); i--;){
        ...
        W.set_vy(0.001*v0*sin(kx*p.x())*exp(-(y-M)*(y-M)/32./d/d));
        if(y > M+d){
            W.set_vx(-v0);
        } else {
            if(y < M-d){
