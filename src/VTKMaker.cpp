...
VTKMaker::VTKMaker(int argc, char **argv){
    ...
    optind = 1;
    opterr = 0;
    while((c = getopt_long(argc, argv, ":t:o:dv", long_options, NULL))
          != -1){
        switch(c){
        case 't':
            readertype = optarg;
        ...
        case 'd':
            ...
            break;
        case 'v':
            // not really necessary, but it is nicer to have this option as well
            // for clarity
            voronoi = true;
            break;
        case ':':
    ...
    SnapshotReader *reader = SnapshotReaderFactory::generate(readertype,
                                                             filename,
                                                             simulation_units);
