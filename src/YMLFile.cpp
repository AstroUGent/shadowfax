...
ParameterFile::ParameterFile(std::string filename) {
    ...
    if(!file) {
        //    cmac_error("Failed to open parameter file \"%s\"",
        //    filename.c_str());
        my_exit();
    }
    ...
    while(getline(file, line)) {
        if(!is_comment_line(line) && !is_empty_line(line)) {
            ...
            if(indentation > 0) {
                // line is indented: check if it is more indented than the
                // previous line
                if(levels.size() > 0) {
                ...
                if(levels.size() != groupname.size()) {
                    //          cmac_error("Line has a different indentation
                    //          than expected: \"%s\"!",
                    //                     line.c_str());
                    my_exit();
                }
                ...
                if(value.empty()) {
                    ...
                    // it contains a value: get the complete key name, which is
                    // composed
                    // of the various parent group names and the actual key name
                    key = "";
                    for(auto it = groupname.begin(); it != groupname.end();
                        ++it) {
                        key += *it + ":";
                    }
...
void ParameterFile::print_contents(std::ostream& stream) const {
    ...
    for(auto it = _dictionary.begin(); it != _dictionary.end(); ++it) {
        ...
        // print group info (if necessary) and get the correct indentation for
        // the
        // line
        std::string indent = "";
