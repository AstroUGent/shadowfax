 ...
 * @file predicates.cpp
 *
 * @brief Arbitrary precision arithmetics geometry tests
 *
 * The code in this file is mostly written by Jonathan Richard Shewchuk, see the
 * detailed comment block in the source file. Some small adaptations were made
 * to make it work with the class structure of Shadowfax.
 *
 * @author Jonathan Richard Shewchuk (jrs@cs.cmu.edu)
 * @author Bert Vandenbroucke (bert.vandenbroucke@ugent.be)
 */

/// @cond We're not going to document code we didn't write ourselves...

#include "predicates.hpp"  // for PredicatesInitializer
#include <cmath>           // for fabs
#include <cstdlib>         // for random
#include <limits>          // for numeric_limits

// some of the functions in this file contain very quick, but also dirty code
// that reads values outside of buffers. Since these values are never actually
// used, there is no problem in doing this (and it is fast)
// however, the GNU sanitizer will complain about these events (as it should)
// by providing these functions with the attribute macro defined below, we
// can turn of the sanitizer for these functions
...
// Initialize library
PredicatesInitializer exact;
double PredicatesInitializer::machine_epsilon =
...
REAL orient2dslow(REAL* pa, REAL* pb, REAL* pc) {
    ...
    Two_Two_Product(acx, acxtail, bcy, bcytail, axby7, axby[6], axby[5],
                    axby[4], axby[3], axby[2], axby[1], axby[0]);
    axby[7] = axby7;
    negate = -acy;
    negatetail = -acytail;
...
double predicates::orient2d_old(double* pa, double* pb, double* pc) {
...
REAL orient3dslow(REAL* pa, REAL* pb, REAL* pc, REAL* pd) {
    ...
    Two_Two_Product(adx, adxtail, bdy, bdytail, axby7, axby[6], axby[5],
                    axby[4], axby[3], axby[2], axby[1], axby[0]);
    axby[7] = axby7;
    negate = -ady;
    negatetail = -adytail;
    Two_Two_Product(bdx, bdxtail, negate, negatetail, bxay7, bxay[6], bxay[5],
                    bxay[4], bxay[3], bxay[2], bxay[1], bxay[0]);
    bxay[7] = bxay7;
    Two_Two_Product(bdx, bdxtail, cdy, cdytail, bxcy7, bxcy[6], bxcy[5],
                    bxcy[4], bxcy[3], bxcy[2], bxcy[1], bxcy[0]);
    bxcy[7] = bxcy7;
    negate = -bdy;
    negatetail = -bdytail;
    ...
    cxby[7] = cxby7;
    Two_Two_Product(cdx, cdxtail, ady, adytail, cxay7, cxay[6], cxay[5],
...
REAL orient3dadapt(REAL* pa, REAL* pb, REAL* pc, REAL* pd, REAL permanent) {
    ...
    if(adxtail != 0.0) {
        if(bdytail != 0.0) {
            ...
            finother = finswap;
            if(cdztail != 0.0) {
                Two_One_Product(adxt_bdyt1, adxt_bdyt0, cdztail, u3, u[2], u[1],
                                u[0]);
                u[3] = u3;
                finlength = fast_expansion_sum_zeroelim(finlength, finnow, 4, u,
                                                        finother);
        ...
        if(cdytail != 0.0) {
            ...
            finother = finswap;
            if(bdztail != 0.0) {
                Two_One_Product(adxt_cdyt1, adxt_cdyt0, bdztail, u3, u[2], u[1],
                                u[0]);
                u[3] = u3;
                finlength = fast_expansion_sum_zeroelim(finlength, finnow, 4, u,
                                                        finother);
    ...
    if(bdxtail != 0.0) {
        if(cdytail != 0.0) {
            ...
            finother = finswap;
            if(adztail != 0.0) {
                Two_One_Product(bdxt_cdyt1, bdxt_cdyt0, adztail, u3, u[2], u[1],
                                u[0]);
                u[3] = u3;
                finlength = fast_expansion_sum_zeroelim(finlength, finnow, 4, u,
                                                        finother);
        ...
        if(adytail != 0.0) {
            ...
            finother = finswap;
            if(cdztail != 0.0) {
                Two_One_Product(bdxt_adyt1, bdxt_adyt0, cdztail, u3, u[2], u[1],
                                u[0]);
                u[3] = u3;
                finlength = fast_expansion_sum_zeroelim(finlength, finnow, 4, u,
                                                        finother);
    ...
    if(cdxtail != 0.0) {
        if(adytail != 0.0) {
            ...
            finother = finswap;
            if(bdztail != 0.0) {
                Two_One_Product(cdxt_adyt1, cdxt_adyt0, bdztail, u3, u[2], u[1],
                                u[0]);
                u[3] = u3;
                finlength = fast_expansion_sum_zeroelim(finlength, finnow, 4, u,
                                                        finother);
        ...
        if(bdytail != 0.0) {
            ...
            finother = finswap;
            if(adztail != 0.0) {
                Two_One_Product(cdxt_bdyt1, cdxt_bdyt0, adztail, u3, u[2], u[1],
                                u[0]);
                u[3] = u3;
                finlength = fast_expansion_sum_zeroelim(finlength, finnow, 4, u,
                                                        finother);
...
double predicates::orient3d_old(double* pa, double* pb, double* pc,
                                double* pd) {
...
REAL incircleadapt(REAL* pa, REAL* pb, REAL* pc, REAL* pd, REAL permanent) {
    ...
    if(adxtail != 0.0) {
        ...
        temp16alen =
                scale_expansion_zeroelim(axtbclen, axtbc, 2.0 * adx, temp16a);
    ...
    if(adytail != 0.0) {
        ...
        temp16alen =
                scale_expansion_zeroelim(aytbclen, aytbc, 2.0 * ady, temp16a);
    ...
    if(bdxtail != 0.0) {
        ...
        temp16alen =
                scale_expansion_zeroelim(bxtcalen, bxtca, 2.0 * bdx, temp16a);
    ...
    if(bdytail != 0.0) {
        ...
        temp16alen =
                scale_expansion_zeroelim(bytcalen, bytca, 2.0 * bdy, temp16a);
    ...
    if(cdxtail != 0.0) {
        ...
        temp16alen =
                scale_expansion_zeroelim(cxtablen, cxtab, 2.0 * cdx, temp16a);
    ...
    if(cdytail != 0.0) {
        ...
        temp16alen =
                scale_expansion_zeroelim(cytablen, cytab, 2.0 * cdy, temp16a);
...
double predicates::incircle_old(double* pa, double* pb, double* pc,
                                double* pd) {
...
REAL insphereslow(REAL* pa, REAL* pb, REAL* pc, REAL* pd, REAL* pe) {
    ...
    Two_Two_Product(aex, aextail, bey, beytail, axby7, axby[6], axby[5],
                    axby[4], axby[3], axby[2], axby[1], axby[0]);
    axby[7] = axby7;
    negate = -aey;
    negatetail = -aeytail;
    ...
    Two_Two_Product(bex, bextail, cey, ceytail, bxcy7, bxcy[6], bxcy[5],
                    bxcy[4], bxcy[3], bxcy[2], bxcy[1], bxcy[0]);
    bxcy[7] = bxcy7;
    negate = -bey;
    negatetail = -beytail;
    ...
    Two_Two_Product(cex, cextail, dey, deytail, cxdy7, cxdy[6], cxdy[5],
                    cxdy[4], cxdy[3], cxdy[2], cxdy[1], cxdy[0]);
    cxdy[7] = cxdy7;
    negate = -cey;
    negatetail = -ceytail;
    ...
    Two_Two_Product(dex, dextail, aey, aeytail, dxay7, dxay[6], dxay[5],
                    dxay[4], dxay[3], dxay[2], dxay[1], dxay[0]);
    dxay[7] = dxay7;
    negate = -dey;
    negatetail = -deytail;
    ...
    Two_Two_Product(aex, aextail, cey, ceytail, axcy7, axcy[6], axcy[5],
                    axcy[4], axcy[3], axcy[2], axcy[1], axcy[0]);
    axcy[7] = axcy7;
    negate = -aey;
    negatetail = -aeytail;
    ...
    Two_Two_Product(bex, bextail, dey, deytail, bxdy7, bxdy[6], bxdy[5],
                    bxdy[4], bxdy[3], bxdy[2], bxdy[1], bxdy[0]);
    bxdy[7] = bxdy7;
    negate = -bey;
    negatetail = -beytail;
...
double predicates::insphere_old(double* pa, double* pb, double* pc, double* pd,
                                double* pe) {
    REAL aex, bex, cex, dex;
    REAL aey, bey, cey, dey;
    REAL aez, bez, cez, dez;
