...
private:
     ...
     */
    template<typename T> unsigned int num_less_than(std::vector<T*>& A,
                                                    unsigned long max){
        Hilbert_Object maxobj(max);
        return distance(A.begin(), lower_bound(A.begin(), A.end(),
                                               &maxobj, HB::sortfunc));
    }
     ...
     * key
     */
    template<typename T> unsigned int num_greater_than(std::vector<T*>& A,
                                                       unsigned long min){
     ...
     */
    template<typename T> unsigned int num_of_occurences(std::vector<T*>& A,
                                                        unsigned long key){
     ...
     */
    template<typename T> unsigned long median(std::vector<T*>& A,
    ...
    unsigned long sparse_median(unsigned long loc_median, bool have_value){
        ...
        for(unsigned int i = p; i--;){
            if(!i){
                ...
                MyMPI_Isend(&send_loc_median, 1, MPI_UNSIGNED_LONG, (r+i)%p, 1,
                            &reqs[1]);
                char msg_bool;
                MyMPI_Irecv(&msg_bool, 1, MPI_CHAR, (r+p-i)%p, 0, &reqs[2]);
     ...
     */
    template<typename T> Splitter find_splitter(unsigned int target_cnt,
                                                std::vector<T*>& A,
                                                Splitter& splitter0,
                                                Splitter& splitterp){
     ...
     */
    template<typename T> Splitter find_splitter(unsigned int target_weight,
                                                std::vector<T*>& A,
...
public:
    ...
    template<typename T> void sort(std::vector<T*>& A){
        ...
        for(unsigned int i = 1; i < p; i++){
            splitter[i] = find_splitter(target_cnt, A, splitter[0],
                    splitter[p]);
        ...
        for(unsigned int i = p; i--;){
            ...
            unsigned int j = 0;
            while(j < A.size() &&
                  (A[j] == NULL || A[j]->get_key() < splitter[(r+i)%p].key())){
                j++;
            }
            ...
            if(i){
                ...
                MyMPI_Isend(&send_sendsize, 1, MPI_INT, (r+i)%p, 0, &reqs[0]);
                MyMPI_Irecv(&recvsize, 1, MPI_INT, (r+p-i)%p, 0, &reqs[1]);
                MPI_Status status[2];
                MyMPI_Waitall(2, reqs, status);

                unsigned int numbuf = MPIGlobal::sendsize/(sizeof(ParticleType) + sizeof(GasParticle));
                unsigned int numsend = sendsize/numbuf + (sendsize%numbuf > 0);
                unsigned int numrecv = recvsize/numbuf + (recvsize%numbuf > 0);
                for(unsigned int bi = 0; bi < std::max(numsend, numrecv); bi++){
                    int sendbufpos[2] = {0,0};
                    for(unsigned int si = bi*numbuf;
                        si < std::min((bi+1)*numbuf, sendsize);
                        si++){
                        ParticleFactory::dump(sendlist[si],
...
//                                                recv_size[0], &recv_pos));
                        newlist.push_back(
                                    ParticleFactory::load(
                                        MPIGlobal::recvbuffer, recv_size[0],
                                        &recv_pos
                                    )
                                );
                    }
