...
void Tree::exchange_pseudonodes(){
    ...
    for(unsigned int i = 0; i < _exportnodes.size(); i++){
        ...
        if(node){
            ...
            exportvector[i] = NodeInfo(_exportnodes[i]->get_key(), 0., 0., 0.,
                                       0., com);
        }
    }
    ...
    for(int i = 0; i < MPIGlobal::size-1; i++){
        int send_pos = 0;
        for(unsigned int si = 0; si < std::min(maxsize, sendsize);
            si++){
        ...
        if(tag == 0){
            ...
            while(recv_pos < nelements-4){
                importvectors[index].push_back(
                            NodeInfo(&MPIGlobal::recvbuffer[freebuffer],
                                     nelements, &recv_pos)
                            );
            }
            int flag;
...
void Tree::add_pseudoparticles(unsigned long keylow, unsigned long keyhigh,
                               unsigned int pindex){
    ((TreeNode*)_root)->add_pseudoparticles(keylow, keyhigh, pindex, 0, 1,
                                            _pseudonodes);
}
        ...
        if(stack->is_leaf()){
            if(stack->is_pseudo()){
                ...
                if(((Leaf*)stack)->get_particle()->type() == PARTTYPE_GAS){
                    if((coords-
                        ((Leaf*)stack)->get_particle()->get_position()).norm2()
                            <= r2){
                        ngblist.push_back((GasParticle*)((Leaf*)stack)
                                          ->get_particle());
                    }
                }
...
void Tree::get_neighbours_outside(Vec &coords, double radius,
                                  ...
                                  vector<bool>& exportlist, unsigned int index){
#if ndim_==3
    if(_periodic){
                ...
                for(unsigned int k = 3; k--;){
                    if(!(i==1 && j==1 && k==1)){
                        ...
                        for(unsigned int l = copy_ngbs.size(); l--;){
                            if(!copy_ngbs[l]->is_copied(i*9+j*3+k+1, index)){
                                ngblist.push_back(
                                            new VorGen(copy_ngbs[l]->x()+
                                ...
                                copy_ngbs[l]->make_copy(i*9+j*3+k+1, index);
                        ...
                        if(i){
                            if(i > 1){
                                coords_copy[0] = 2.*(_box.get_anchor()[0]+
                                        _box.get_sides()[0])-coords_copy[0];
                            }
                        } else {
                            coords_copy[0] = 2.*_box.get_anchor()[0]-
                                    coords_copy[0];
                        ...
                        if(j){
                            if(j > 1){
                                coords_copy[1] = 2.*(_box.get_anchor()[1]+
                                        _box.get_sides()[1])-coords_copy[1];
                            }
                        } else {
                            coords_copy[1] = 2.*_box.get_anchor()[1]-
                                    coords_copy[1];
                        ...
                        if(k){
                            if(k > 1){
                                coords_copy[2] = 2.*(_box.get_anchor()[2]+
                                        _box.get_sides()[2])-coords_copy[2];
                            }
                        } else {
                            coords_copy[2] = 2.*_box.get_anchor()[2]-
                                    coords_copy[2];
            ...
            for(unsigned int j = 3; j--;){
                if(!(i==1 && j==1)){
                    ...
                    if(i){
                        ...
                        coords_copy[0] = 2.*_box.get_anchor()[0]-coords_copy[0];
                    }
                    if(j){
                        if(j > 1){
                            coords_copy[1] = 2.*(_box.get_anchor()[1]+
                        ...
                        coords_copy[1] = 2.*_box.get_anchor()[1]-coords_copy[1];
                    }
                    vector<GasParticle*> copy_ngbs;
                    get_neighbours(coords_copy, radius, copy_ngbs, exportlist);
                    for(unsigned int k = copy_ngbs.size(); k--;){
...
Particle* Tree::get_closest(Vec &coords, double r){
    ...
    while(!closest){
        ...
        while(current){
            if(current->is_leaf()){
                double check = (((Leaf*)current)->get_particle()
                                ->get_position()-coords).norm2();
                if(check < r2){
...
void TreeNode::add_pseudoparticles(unsigned long keylow, unsigned long keyhigh,
    ...
    for(unsigned int i = numnode_; i--;){
        ...
        if(interval_overlap(keylow, keyhigh, subnodekey, subnodekey+keysize)){
            Vec anchor = _box.get_anchor();
            ...
            if(_nodes[i] == NULL){
                if(keylow <= subnodekey && subnodekey+keysize <= keyhigh){
                    ...
                    ((TreeNode*)_nodes[i])->add_pseudoparticles(keylow, keyhigh,
                                                                pindex, level+1,
                                                                (key<<ndim_)+i,
                                                                pseudonodes);
                }
    ...
    for(unsigned int i = numnode_; i--;){
        ...
        if(interval_overlap(keylow, keyhigh, subnodekey, subnodekey+keysize)){
            if(_nodes[i] == NULL){
                ...
                if(keylow <= subnodekey && subnodekey+keysize <= keyhigh){
                    exportnodes.push_back(new ExportNode(_nodes[i],
                                                         subnodekey));
                } else {
                    ...
                    if(!_nodes[i]->is_leaf()){
                        ((TreeNode*)_nodes[i])->get_exportnodes(keylow, keyhigh,
                                                                level+1,
                                                                (key<<ndim_)+i,
                                                                exportnodes);
                    }
...
void TreeNode::update_quantities(bool all){
    ...
    while(current){
        ...
        for(unsigned int i = ndim_; i--;){
            _center_of_mass[i] += current->get_mass()*
                    current->get_center_of_mass(i);
        }
